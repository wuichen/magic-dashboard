...
export function AuthProvider({ children }) {
  ...
  async function setAuthentication(json) {
    ...
    Cookies.set('token', jwt, { domain: process.env.NEXT_PUBLIC_COOKIE_DOMAIN });
    set_state({
      ...state,
      loginRequestId,
      user,
  ...
  async function logout() {
    ...
    await fetch('/api/magic/logout', { method: 'POST' });
    // window.location = '/';
  }
  ...
  async function refreshTokens() {
    ...
    async function handleRefreshTokens() {
      try {
        const response = await fetch('/api/magic/refresh', { method: 'POST' });
